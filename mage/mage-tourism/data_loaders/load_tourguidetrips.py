...
def load_from_mongodb(*args, **kwargs):
    ...
    documents = db['tourguidetrips'].find()
    data = list(documents)
    ...
    df.rename(columns={'ticketPerPerson': 'ticket_per_person'}, inplace=True)
    df.rename(columns={'minimumNumber': 'minimum_number'}, inplace=True)
    df.rename(columns={'createdBy': 'created_by'}, inplace=True)
    df.rename(columns={'createdAt': 'created_at'}, inplace=True)
    df.rename(columns={'updatedAt': 'updated_at'}, inplace=True)
    df.rename(columns={'maximumNumber': 'maximum_number'}, inplace=True)
    df.rename(columns={'currentTravelersNo': 'current_travelers_no'}, inplace=True)
    df.rename(columns={'VIP': 'vip'}, inplace=True)
