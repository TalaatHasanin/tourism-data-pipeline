...
def load_from_mongodb(*args, **kwargs):
    ...
    data = list(db['entertainmentmp_places'].find())
    ...
    df = df.drop(columns=['__v', 'customId'])

    # df.rename(columns={'touristId': 'tourist_id'}, inplace=True)

    image_df = pd.json_normalize(df['image'])
    df = df.drop(columns=['image']).join(image_df)

    location_df = pd.json_normalize(df['location'])
    df = df.drop(columns=['location']).join(location_df)
